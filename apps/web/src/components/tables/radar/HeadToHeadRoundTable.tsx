        ...
        <div className="flex flex-col w-full space-y-2">
          <div className="border-b border-dashed border-gray-400 dark:border-gray-200/40 pb-4 mb-2">
            ...
            <p className="text-sm italic">
              Based on {numRounds || "--"} rounds, {code} has a{" "}
              <TooltipTrigger className="italic text-blue-400">
                clutch factor
              </TooltipTrigger>{" "}
              of{" "}
          ...
          <div className="w-full justify-between flex">
            <p className="text-sm">
              ...
              {isFavorite ? `≥ ${matchupWinPct}%` : `≤ ${matchupWinPct}%`}{" "}
          ...
          {filteredRounds === undefined || filteredRounds.length ? (
                ...
                core: [
                  ...
                  column.display({
                    ...
                    cell: (props) => {
                      ...
                      let expHiWinProp =
                        Math.floor(getExpectedWP(otr, opponentOtr, 10) * 1000) /
                        10;
            ...
            <p className="text-sm text-red-400 text-center max-w-[500px] mx-auto">
              {code} has had no rounds as a
              {isFavorite ? " favorite" : "n underdog"} (with an expected win
              probability of{" "}
              {isFavorite ? "as much or higher" : "as little or less"} than in
              this matchup) where they've{" "}
              {teamOutcome === "Wins" ? "won" : "lost"}.
            </p>
          )}
        </div>
        <TooltipContent>
          A composite score of winning rounds that you're an underdog in and
          losing rounds that you're favored in. Represents how "clutch" you are
          in conforming to expected win percentages, with 10 being the best.
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
